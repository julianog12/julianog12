...
class ProcessarTrigger
  ...
  conteudo.include?(';Includeablockliketheoneshownbelowattheendofany;Procwritteninthistrigger.;...;if($error=0139);putmess$text(0139);return(-1);else;message$text("0000");return(-1);endif'.encode('UTF-8', :invalid => :replace, :undef => :replace, :replace => "?")) ||
  ...
  def possui_include?(linha, v_linha)
    posic_include = 0
    posic_include = ((linha.index("include LIB_COAMO:")||0)+(linha.index("include COAMO_LIB:")||0))  if !v_linha.nil? && linha[0..1] == '[ ' && !v_linha.match(/^\;/) && !v_linha.include?('defparam')
